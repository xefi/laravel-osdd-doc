...
4. Strip `Database\Factories\` and `Database\Seeders\` PSR-4 entries from root `composer.json` (the `App\` mapping is kept so Laravel can still detect an application namespace)
